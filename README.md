...
| Path | Purpose |
|------|---------|
| `unifi_mcp/server.py` | FastMCP server, destructive-ops gate, Bearer middleware |
| `unifi_mcp/client.py` | Controller API client, UDM Pro vs legacy auth |
| `unifi_mcp/config.py` | `UniFiConfig` and `ServerConfig` dataclasses |
| `unifi_mcp/models/enums.py` | `UnifiAction` enum and action-set constants |
| `unifi_mcp/models/params.py` | `UnifiParams` Pydantic model |
| `unifi_mcp/services/` | Domain service layer (device, client, network, monitoring) |
| `unifi_mcp/tools/` | Individual tool implementations |
| `unifi_mcp/formatters.py` | Shared formatters for bytes, timestamps, tables |
| `skills/unifi/SKILL.md` | Skill action reference and operational guidance |
