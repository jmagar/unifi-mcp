...
def register_monitoring_tools(mcp: FastMCP, client: UnifiControllerClient) -> None:
    ...
    async def get_controller_status() -> ToolResult:
        ...
        try:
            ...
            resp = {"status": "online", "server_version": result.get("server_version", "Unknown"), "up": result.get("up", False), "details": result}
    ...
    async def get_events(limit: int = 100, site_name: str = "default") -> ToolResult:
        ...
        try:
            ...
            return ToolResult(content=[TextContent(type="text", text=summary_text)], structured_content=formatted_events)
    ...
    async def get_alarms(active_only: bool = True, site_name: str = "default") -> ToolResult:
        ...
        try:
            ...
            return ToolResult(content=[TextContent(type="text", text=summary_text)], structured_content=formatted_alarms)
    ...
    async def get_dpi_stats(by_filter: str = "by_app", site_name: str = "default") -> ToolResult:
        ...
        try:
            ...
            if isinstance(dpi_stats, dict) and "error" in dpi_stats:
                return ToolResult(content=[TextContent(type="text", text=f"Error: {dpi_stats.get('error', 'unknown error')}")], structured_content=[dpi_stats])
            ...
            return ToolResult(content=[TextContent(type="text", text=summary_text)], structured_content=formatted_stats)
    ...
    async def get_rogue_aps(site_name: str = "default", limit: int = 20) -> ToolResult:
        ...
        try:
            ...
            if isinstance(rogue_aps, dict) and "error" in rogue_aps:
                return ToolResult(content=[TextContent(type="text", text=f"Error: {rogue_aps.get('error', 'unknown error')}")], structured_content=[rogue_aps])
            ...
            filtered_rogues = sorted(rogue_aps, key=lambda x: x.get("rssi", -100), reverse=True)[:limit]
            ...
            if len(rogue_aps) > limit:
                formatted_rogues.append({"summary": f"Showing top {limit} of {len(rogue_aps)} detected rogue APs (sorted by signal strength)"})
            ...
            if header:
                summary_text = header + "\n" + summary_text
            return ToolResult(content=[TextContent(type="text", text=summary_text)], structured_content=formatted_rogues)
    ...
    async def start_spectrum_scan(mac: str, site_name: str = "default") -> ToolResult:
        ...
        try:
            ...
            if isinstance(result, dict) and "error" in result:
                return ToolResult(content=[TextContent(type="text", text=f"Error: {result.get('error', 'unknown error')}")], structured_content=result)

            resp = {"success": True, "message": f"Spectrum scan started on AP {mac}", "details": result}
            return ToolResult(content=[TextContent(type="text", text=f"Spectrum scan started: {mac}")], structured_content=resp)
    ...
    async def authorize_guest(
        mac: str, minutes: int = 480, up_bandwidth: int | None = None, down_bandwidth: int | None = None, quota: int | None = None, site_name: str = "default"
    ) -> ToolResult:
        """
        ...
        try:
            ...
            if minutes <= 0:
                return ToolResult(content=[TextContent(type="text", text="Error: minutes must be > 0")], structured_content=[{"error": "invalid_minutes"}])
            for k, v in (("up", up_bandwidth), ("down", down_bandwidth), ("bytes_mb", quota)):
                if v is not None and v < 0:
                    return ToolResult(
                        content=[TextContent(type="text", text=f"Error: {k} must be non-negative")], structured_content=[{"error": f"invalid_{k}"}]
                    )
            ...
            resp = {"success": True, "message": f"Guest {mac} authorized for {minutes} minutes", "details": result}
    ...
    async def get_speedtest_results(site_name: str = "default", limit: int = 20) -> ToolResult:
        ...
        try:
            ...
            data = {"start": start_time, "end": end_time, "attrs": ["time", "xput_download", "xput_upload", "latency", "ping", "jitter"]}

            results = await client._make_request("POST", "/stat/report/archive.speedtest", site_name=site_name, data=data)

            if isinstance(results, dict) and "error" in results:
                return ToolResult(content=[TextContent(type="text", text=f"Error: {results.get('error', 'unknown error')}")], structured_content=[results])
            ...
            return ToolResult(content=[TextContent(type="text", text=summary_text)], structured_content=formatted_results)
    ...
    async def get_ips_events(site_name: str = "default", limit: int = 50) -> ToolResult:
        ...
        try:
            ...
            data = {
                "start": start_time,
                "end": end_time,
                "attrs": ["time", "src_ip", "dst_ip", "proto", "app_proto", "signature", "category", "action", "severity", "msg"],
            }

            events = await client._make_request("POST", "/stat/ips/event", site_name=site_name, data=data)
